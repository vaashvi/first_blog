        ...
        if form.is_valid():
            ...
            if user is not None:
                if user.is_active:
                    ...
                    return render(request, 'promeds/menu.html', {'user': user})
                else:
...
def login(request):
    ...
    if request.method == 'POST':
        ...
        if not username:
            if not password:
                return render(request, 'promeds/trialhome.html', {'error_message': 'Please enter the Username and Password'})
            else:
                return render(request, 'promeds/trialhome.html', {'error_message': 'Please enter the Username'})
        elif not password:
        ...
        if user is not None:
            if user.is_active:
                ...
                return render(request,'promeds/trialhome.html',{'error_message':'Your account has been disabled'})
        else:
            return render(request, 'promeds/trialhome.html', {'error_message': 'Username and Password do not match'})
    else:
...
def healthcarepage(request):
    ...
    return render(request,'promeds/firstaid.html',{   'all_medicines': all_medicines ,'all_subcategory': all_subcategory})
...
def subcategorypage(request, subcat_id):
    ...
    all_subcategory = category.objects.all()
    value = 1
    return render(request, 'promeds/firstaid.html', {'all_medicines': all_medicines, 'all_subcategory': all_subcategory , 'value':value})
...
def addtocartpage(request,medid):
    ...
    return render(request,'promeds/firstaid.html', {'all_medicines': all_medicines ,'all_subcategory': all_subcategory , 'medicinename':medicinename ,'value':value })
...
def index(request):
    ...
    context = {'all_tablets': all_tablets}
    return render(request,'promeds/ordertab.html', context)

def detailTablet(request, tablet_id):
    #return HttpResponse(tablet_id)
    ...
    tablet = Tablets.objects.get(pk = tablet_id)
    #return HttpResponse("hi"+ tablet.tab_name)
    return render(request,'promeds/tabpage.html',{'tablet':tablet , 'value':value})
...
def reviewpage(request):
    ...
    print(shp.landmark)
    shp.save()
    return render(request,'promeds/revieworder.html',{'tabletss':tabletss,'tab':tab})
...
def deletemed(request,medd_id):
    ...
    return render(request, 'promeds/reviewmeds.html', {'medss': medss, 'med': med})
...
def billpage(request):
    ...
    return render(request,'promeds/billdet.html',{'tabletss' : tabletss ,'tab':tab , 'bills':bills,'user':user,'t1':t1,'spd':spd })
...
def billmedpage(request):
    ...
    return render(request, 'promeds/billmed.html',
                  {'medss': medss, 'med': med, 'bills': bills, 'user': user, 't2': t2 ,"spd":spd})
...
#     bills.userr = user
#     bills.totalprice = t1
#     bills.billdate = datetime.datetime.utcnow()
#     bills.save()
#     bills.orders.set(tabletss)
#     return render(request,'promeds/billdet.html',{'tabletss' : tabletss ,'tab':tab , 'bills':bills,'user':user,'t1':t1 })
...
def ProfileView(request):
    ...
    return render(request, 'promeds/profile.html', {'profiles': upro})
